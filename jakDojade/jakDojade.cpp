...
void bfs(const int n,
  ...
  while (!q.empty()) {
    ...
    for (int k = 0; k < 4; k++) {
      ...
      if (!inside_map(new_i, new_j, n, m) || arr[new_i * m + new_j] == -2 ||
          explored[new_i * m + new_j])
        continue;
...
void get_city_name(const int n,
                   const int m,
                   Vector<char>& arr,
                   Vector<int>& normal_array,
                   HashMap_id& hash_id,
                   HashMap_str& hash_city,
                   Vector<JiI>& cities) {
